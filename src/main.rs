...
fn main() {
    // currency_exchange();
    serch_dir("./".to_string()).unwrap();
}


fn serch_dir(path_dir_fn: String) -> std::io::Result<()>{
    ...
    while let Some(path) = stack.pop() {
        for entry in read_dir(&path)? {
            ...
            if entry_path.is_dir() {
                ...
                dir_list.push(entry_path.clone().display().to_string());
            } else if entry_path.is_file() {
                println!("Файл: {}", entry_path.clone().display());
                file_list.push(entry_path.clone().display().to_string());
            }
        }
    }
    Ok(())
